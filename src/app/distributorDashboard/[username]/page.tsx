...
// Updated Crop interface based on provided schema
interface Crop {
    _id: { $oid: string };
    cropId: string;
    cropName: string;
...
const DistributorDashboard = () => {
    ...
    const [distributorCrops, setDistributorCrops] = useState<Crop[]>([]); // ✅ Initialized as array
    ...
    const fetchDistributorCrops = async () => {
        ...
        try {
            ...
            const crops = response.data.crops || response.data; // fallback if 'crops' key is missing
            setDistributorCrops(Array.isArray(crops) ? crops : [crops]); // ✅ Normalize to array
        } catch (error: any) {
    ...
    const handleUpdateCrop = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const errorMessage = error.response?.data?.message || 'Failed to update crop.';
            setFormStatus({ message: errorMessage, type: 'error' });
        }
    };

    const tableHeaderClass = "px-6 py-3 text-left text-xs font-medium text-purple-100 uppercase tracking-wider";
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="bg-stone-800 rounded-lg shadow-xl p-6 mb-12 border border-purple-700">
                    ...
                    <div className="flex flex-col items-center gap-4">
                        <p className="text-center text-stone-400">Upload a QR code image to get the Crop ID.</p>
                        <input
                            type="file"
                            accept="image/*"
                ...
                <div className="bg-stone-800 rounded-lg shadow-xl p-6 mb-12 border border-purple-700">
                    ...
                    <form onSubmit={handleUpdateCrop} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium mb-1">Distributor Date</label>
                                <input
                                    type="date"
                            ...
                            <div>
                                <label className="block text-sm font-medium mb-1">Distributor Location</label>
                                <input
                                    type="text"
                ...
                <div className="bg-stone-800 rounded-lg shadow-xl p-6 border border-purple-700">
                    <h2 className="text-2xl font-bold mb-6 text-purple-200">My Handled Crops</h2>
                    {loading ? (
                        <p className="text-center text-stone-400">Loading crops...</p>
                    ) : distributorCrops.length === 0 ? (
                        <p className="text-center text-stone-400">No crops recorded by this distributor yet.</p>
                    ) : (
                            ...
                            <table className="min-w-full divide-y divide-stone-700">
                                <thead className="bg-stone-700">
                                    <tr>
                                        ...
                                        <th scope="col" className={tableHeaderClass}>Farmer #</th>
                                        <th scope="col" className={tableHeaderClass}>Farmer Username</th>
                                        <th scope="col" className={tableHeaderClass}>Price</th>
                                        <th scope="col" className={tableHeaderClass}>Distributor Price</th>
                                        <th scope="col" className={tableHeaderClass}>Distributor Date</th>
                                        <th scope="col" className={tableHeaderClass}>Delivery Name</th>
                                        <th scope="col" className={tableHeaderClass}>Delivery Number</th>
                                    </tr>
                                </thead>
                                <tbody className="bg-stone-800 divide-y divide-stone-700">
                                    {distributorCrops.map((crop) => (
                                        <tr key={crop._id.$oid} className="hover:bg-stone-700 transition-colors duration-150">
                                            ...
                                            <td className={tableDataClass}>{crop.farmerNumber.$oid}</td>
                                            <td className={tableDataClass}>{crop.farmerUsername.$oid}</td>
                                            <td className={tableDataClass}>${crop.price}</td>
                                            <td className={tableDataClass}>${crop.distributorPrice}</td>
                                            <td className={tableDataClass}>{new Date(crop.distributorDate.$date).toLocaleDateString()}</td>
                                            <td className={tableDataClass}>{crop.distributorDeliveryName}</td>
                                            <td className={tableDataClass}>{crop.distributorDeliveryNumber}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
