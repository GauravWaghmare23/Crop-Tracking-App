...
export default function DistributorDashboard() {
  ...
  const fetchCrops = async () => {
    try {
      const res = await axios.get("/api/crops/distributor/fetch");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        distributorDeliveryNumber: Number(formData.distributorDeliveryNumber),
      };
      const res = await axios.put("/api/crops/distributor/add", payload);
      setFormStatus({ message: res.data.message, type: "success" });
      setFormData({
        cropId: "",
        distributorPrice: "",
        distributorDate: "",
    ...
    <div className="min-h-screen bg-gradient-to-b from-emerald-100 to-emerald-50">
      <div className="px-6 pt-8 pb-2">
        ...
        <p className="text-emerald-900/80 text-lg mt-2 mb-4">
          Update crop distribution records, scan QR tags, and export your handled entries.
        </p>
      </div>
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 p-4">
        ...
        <div>
          ...
          <div className="bg-white/95 border border-emerald-200 rounded-2xl shadow-lg p-8">
            <h2 className="text-2xl font-bold mb-4 text-emerald-900">Update Crop Distribution</h2>
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <div>
                <label className="text-sm font-semibold text-emerald-800 block mb-1">Distributor Price</label>
                <input
                  type="number"
              ...
              <div>
                <label className="text-sm font-semibold text-emerald-800 block mb-1">Distributor Date</label>
                <input
                  type="date"
              ...
              <div>
                <label className="text-sm font-semibold text-emerald-800 block mb-1">Distributor Location</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-sm font-semibold text-emerald-800 block mb-1">Delivery Name</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-sm font-semibold text-emerald-800 block mb-1">Phone</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-sm font-semibold text-emerald-800 block mb-1">Delivery Number</label>
                <input
                  type="number"
        ...
        <div className="bg-white/95 border border-emerald-200 rounded-2xl shadow-lg p-8 h-full flex flex-col">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-emerald-900">Distributed Crops</h2>
            <button
              onClick={fetchCrops}
          ...
          {crops.length > 0 ? (
              ...
              {crops.map((crop) => (
                ...
                >
                  <div className="flex items-start justify-between">
                    <span>
                      <span className="font-bold text-emerald-800 text-xl">{crop.cropName}</span>
                    ...
                    <div className="text-right">
                      <span className="block text-base font-semibold text-emerald-700">
                        Qty: <span className="font-bold">{crop.quantity}</span> <span className="text-xs">kg</span>
                      </span>
                      <span className="block text-base font-semibold text-emerald-700">
                        Price: <span className="font-bold">{crop.price}</span>
                      </span>
                      <span className="block text-base font-semibold text-teal-800">
                        D.Price: <span className="font-bold">{crop.distributorPrice}</span>
                      </span>
                    </div>
                  </div>
                  <div className="text-gray-700 mb-2 text-sm">
                    <span className="block">Location: {crop.distributorLocation}</span>
                    <span>
                      Harvest: {new Date(crop.harvestDate).toLocaleDateString()}
                      <br />
                      Expiry: {new Date(crop.expiryDate).toLocaleDateString()}
                      <br />
                      Distribution: {new Date(crop.distributorDate).toLocaleDateString()}
                    </span>
                  </div>
                  <div className="flex items-center gap-4 mt-2">
                    <div id={`qrcode-${crop.cropId}`} className="bg-white rounded shadow p-2">
            ...
            <p className="text-center text-emerald-800/70">No distributed crops found.</p>
          )}
        </div>
      </div>
